...
from microconventions.leaderboard_conventions import LeaderboardVariety, Genus
import numpy as np
...
def tear_down(rdz, target, target_key, model_key, model_key1, model_key2, model_key3, num_exec=0):
    ...
    rdz.admin_shrinkage()
    memory_leaderboards = dict(
        [(memory, rdz.get_leaderboard(variety=LeaderboardVariety.memory, memory=memory)) for memory in
         rdz.LEADERBOARD_MEMORIES])
    for delay in rdz.DELAYS:
        for genus in Genus:
            lb = rdz.get_leaderboard(variety=LeaderboardVariety.sponsor, sponsor=target_code)
            lb = rdz.get_leaderboard(variety=LeaderboardVariety.sponsor_and_genus, sponsor=target_code, genus=genus)
