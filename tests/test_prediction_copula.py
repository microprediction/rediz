...
def feed(rdz, targets, write_key):
    ...
    res = rdz.cset(names=targets, write_key=write_key, values=values)
...
def tear_down(rdz, targets, target_key, model_key, model_key1, model_key2, model_key3, num_exec=0):
    for target in targets:
        ...
        report = {"promises_delivered": num_exec,
                  ...
                  "leaderboards": dict(
                      [(target + str(delay), rdz.get_leaderboard(variety=LeaderboardVariety.name_and_delay, name=target, delay=delay)) for delay in rdz.DELAYS]),
...
def do_serial(rdz):
    ...
    samples = rdz.get_samples(name=targets[0], delay=1)
    ...
    for _ in range(5 * 5):
        ...
        samples = rdz.get_samples(name=target[0], delay=1)

    tear_down(rdz=rdz, targets=targets, target_key=target_key, model_key=model_key, model_key1=model_key1,
              model_key2=model_key2, model_key3=model_key3, num_exec=num_exec)
