...
def test_propagation():
    ...
    rdz.set( name = SUBSCRIBER, value = "some value",  write_key=SUBSCRIBER_write_key )["write_key"]
    assert rdz.set( name = SUBSCRIBER, value = "some value",       write_key=SUBSCRIBER_write_key )["write_key"] is not None
    assert rdz.set( name = PUBLISHER,  value = "some other value", write_key=PUBLISHER_write_key )["write_key"] is not None
    rdz._subscribe( publisher = PUBLISHER, subscriber = SUBSCRIBER )
    assert rdz.set( name = PUBLISHER, value = "some new value",    write_key=PUBLISHER_write_key )["write_key"] is not None
...
def test_delay():
    ...
    assert rdz.set( name = NAME,  value = "living in the present",  write_key=WRITE_KEY )["write_key"] is not None
    time.sleep(2)
    assert rdz.admin_promises()>0

    delayed_value = rdz.client.get( rdz.DELAYED+"1"+rdz.SEP+NAME )
    dump( {"delayed_value":delayed_value,"expected":"living in the present"} )
